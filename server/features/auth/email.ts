...
export async function sendVerificationEmail(email: string, code: string) {
  await sendEmail(
    email,
    'Your DestacAI verification code',
    `
      <div style="font-family: Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #f9fafb; border-radius: 12px;">
        <h1 style="font-size: 20px; color: #111; margin: 0 0 16px;">Verify your DestacAI account</h1>
        <p style="font-size: 15px; color: #555; margin: 0 0 24px;">Enter this code in DestacAI to verify your account. Expires in 1 hour.</p>
        ...
        <p style="font-size: 13px; color: #9ca3af; margin: 24px 0 0;">If you didn't create an account, you can ignore this email.</p>
      </div>
    `,
  )
}
